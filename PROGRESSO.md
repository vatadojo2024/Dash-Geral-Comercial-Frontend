...
- **Inscritos ≠ assistiram (achado do dado real, 15/09):** `totais.no_evento` do backend conta INSCRITOS (tag WG), não quem assistiu. Pedido ao backend: `totais.assistiram` (tags "Participou" / "Pós WG - DD.MM.AA" / "Levantou a Mão", normalizadas). O front já aceita o campo OPCIONAL: com ele, KPIs e funil viram 4 etapas (Inscritos → Assistiram → Levantou a mão → Agendou) e "% levantaram" passa a ser sobre quem assistiu; sem ele, a tela segue com 3 etapas sobre `no_evento` (`etapasDoFunil` / `baseDeLevantaram`, testados).
- **V3 (17/09, spec `specs/spec_front_levantou_a_mao_v3.md`):** contrato ganhou `matriz` (ao_vivo/replay/total), `resgate` (nullable), `atribuicao_parcial`, `totais.sem_origem` e, por lead, `origem`/`convidado_resgate`/`acessou_replay`/`assistiu_replay` — tudo OPCIONAL no front (sem `matriz`, a aba cai nos KPIs da V2). A matriz (`MatrizOrigem`) SUBSTITUI a linha de KPIs no container `p-4 flex flex-col … sm:flex-row`: tabela compacta em `sm:`, linhas empilhadas em tela estreita (wrapper `contents`); null → travessão (`celulaMatriz`/`formatarTaxa`), recorte todo zerado vira travessões (`linhaSemDados`, ex.: ciclo sem replay), Total com peso maior e não clicável. Clique na linha e chips de origem compartilham o MESMO estado (`origensSel`); "Convidados de resgate" é filtro independente (`soResgate`). Desqualificados e sem_origem viraram notas de conferência ao lado da matriz. `FunilBlocos` é o funil genérico: o do ciclo calcula as taxas, o `FunilResgate` usa as taxas PRONTAS do backend e escala própria (base = convidados). Coluna "Origem" (`OrigemBadge` + marcadores Resgate / Viu o replay também), ordenável com "Sem origem" no fim. CSV: `nome, mql, origem, resgate, dono, etapa, telefone, email, evento, entrou_em, url_clint, tags`. Mock: `?simular=sem_replay|sem_resgate`; a matriz do mock fecha Total = Ao vivo + Replay + Sem origem (`matrizFecha`, testado).
- **V2 (16/09, spec `specs/spec_front_levantou_a_mao_v2.md`):** contrato ganhou `dono`, `etapa`, `url_clint`, `clint_deal_id` por lead, `totais.desqualificados` e `por_dono` — TODOS opcionais no front (backend V1 segue válido). KPI "Desqualificados" neutro (`KpiChip tom="neutro"`), coluna "Dono" ordenável (`DonoBadge`; "Sem dono" sempre no fim nas duas direções), chips de dono (mesmo componente `Chips` dos chips de MQL, contagens de `por_dono`), alternador "Lista | Por SDR" (`Alternador`, o mesmo do Ciclo | Intervalo) com seções por dono (pendentes desc, "Sem dono" último, ordem interna do backend), botão "Abrir na Clint" (`LinkClint`, só com `url_clint`, nunca monta URL), CSV com colunas fixas `nome, mql, dono, etapa, telefone, email, evento, entrou_em, url_clint, tags`, estado "Nenhum lead ativo neste ciclo" (`todosDesqualificados`). Mock: `?simular=desqualificados`, 3 donos + sem dono, 3 em 4 com negócio.
- **Painel lateral do pendente (caminho 1, 15/09):** clicar no nome abre um drawer (`DetalhePendente`) com MQL, evento, telefone (copiar + WhatsApp), e-mail (mailto + copiar), entrada, TODAS as tags da Clint e o id do contato — tudo da própria resposta, sem backend. Motivo: em produção nenhum pendente tinha `lead_id` (o lead só nasce no webhook por mudança de estágio; quem só levantou a mão não existe em `leads` — confirmado por consulta de leitura na base). O link para `/leads/:id` fica dentro do drawer quando `lead_id` vem.
- **Link para a ficha (`leads[].lead_id`, opcional):** backend casa o pendente com a tabela `leads` por e-mail/telefone e devolve o id; o nome vira link para `/leads/:id` (ícone de ficha). null = sem ficha (esperado para quem só se inscreveu). Regra de "agendou" no backend passou a ser call em QUALQUER estágio (decisão Vata 15/09) — texto da aba diz "call agendada", não "1ª call".
- **Mock:** `src/lib/mock/eventos_oportunidades.ts` gera 40 pendentes (1 em 3 com `lead_id` do mock de leads) (7 tiers) distribuídos pelas terças do intervalo; `?simular=vazio|sem_contatos|clint_auth|clint_indisponivel|intervalo_muito_grande` (só em mock) exercita os estados.
