...
- **BUG 2 — Filtros do mapa/fila zeravam a lista:** a lógica de filtro (`filtros.ts`) JÁ comparava contra os valores canônicos (temperatura/etapa certos) — a raiz era o **adapter da lista** (`apiLeads.ts`), escrito para a lista enxuta antiga, que **descartava** `produto_sugerido` (hardcoded `null`), `closer_id`, `sdr_id`, `sdr_pool`, `score_bruto`, `tier_final`. Com todo lead em `produto_sugerido=null`, **o filtro de produto zerava**; para closer/SDR, o re-filtro de escopo client-side esvaziava a fila (posse sempre null). **Fix:** o adapter agora LÊ esses campos da lista real quando presentes (`.nullish()` com default seguro — compatível com a enxuta). 5 testes em `filtros.test.ts` provam que cada temperatura/etapa/produto canônico REDUZ a lista sem zerar e que `etapa_atual=null` cai em "sem_etapa" sem sumir dos outros filtros; +2 testes no adapter da lista cobrindo o enriquecimento. **82 testes no total.**
