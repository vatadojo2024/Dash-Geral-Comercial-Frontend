...
- **BLOQUEADOR (backend):** `https://mapacalor-api.infradojo.pro` responde **404 (Go default) em TODOS** os caminhos testados — `/api/me`, `/api/leads`, `/api/leads/:id`, `/`, `/health`, `/v1/*` etc. O front está correto contra o contrato e degrada com erro claro (502 "A API respondeu 404"), mas o **load real depende das rotas existirem na API** (ou do prefixo correto). Pendente: confirmar com o backend as rotas reais + liberar `FRONT_ORIGIN` (mesmo que CORS não seja acionado pelo proxy, o Supabase precisa do projeto correto).
